...
namespace PCProps::HeatCapacity
{
    ...
    double AlyLee::derivativeOfCp(double temperature) const
    {
        ...
        return (
            2 * m_B * pow(m_C, 3) * cosh(m_C / temperature) / pow(temperature, 4) / pow(sinh(m_C / temperature), 3) -
    ...
    double AlyLee::integralOfCp(double temperature) const
    {
        ...
        return (m_A * temperature + m_B * m_C / tanh(m_C / temperature) - m_D * m_E * tanh(m_E / temperature));
    }
