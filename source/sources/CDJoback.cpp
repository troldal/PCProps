...
namespace
{
    using CDJobackGroup = std::tuple<std::string,
                                     std::optional<double>,
                                     std::optional<double>,
                                     std::optional<double>,
                                     std::optional<double>,
    ...
    const std::array<CDJobackGroup, 41> JobackGroups {
        ...
        CDJobackGroup { "-N= (non-ring)", 0.0255, -0.0099, std::nullopt, 74.60, std::nullopt, 23.61, std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::nullopt, 3.335, std::nullopt, std::nullopt },
        ...
        CDJobackGroup { "=NH (general)", std::nullopt, std::nullopt, std::nullopt, 83.08, 68.91, 93.70, 119.66, 5.69, -4.12E-3, 1.28E-4, -8.88E-8, std::nullopt, 12.169, std::nullopt, std::nullopt },
    ...
    std::optional<double> accumulate(const std::vector<std::pair<int, int>>& groups)
    {
        ...
        for (const auto& item : groups)
            items.emplace_back(std::get<index>(JobackGroups.at(item.second - 1)).has_value() ? std::optional(static_cast<double>(item.first) * std::get<index>(JobackGroups.at(item.second - 1)).value()) : std::nullopt);
...
namespace PCProps::ConstantData
{
    ...
    CDJoback::CDJoback(const std::vector<std::pair<int, int>>& groups, double molecularWeight, int atomCount)
